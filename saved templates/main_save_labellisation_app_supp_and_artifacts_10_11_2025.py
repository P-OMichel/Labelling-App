...
class SpectrogramViewer(QtWidgets.QMainWindow):
    ...
    def load_npy(self):
        ...
        mask = rng.integers(0, 2, size=n, dtype=int)  # 0/1 mask
        masked_signal = self.signal.copy()
        masked_signal[mask == 0] = np.nan
    ...
    def _update_spectrogram_image(self):
        ...
        if np.any(finite):
            p1, p99 = np.percentile(Sxx_db[finite], [1, 99])
            self.img_item.setImage(Sxx_db, autoLevels=False, levels=(float(p1), float(p99)))
        else:
    ...
    def update_overlay(self):
        if self.base_overlay is None or self.f is None or self.t_spec is None:
            return
        thr = float(self.threshold_spin.value())  # 0..1

        # Filter: keep only portions above threshold
        mask = self.base_overlay >= thr
        curve = np.where(mask, self.base_overlay, np.nan)

        # Map curve [0,1] to frequency range [min(f), max(f)]
        fmin, fmax = float(self.f[0]), float(self.f[-1])
        y = fmin + curve * (fmax - fmin)

        # Update curve plot
        self.overlay_curve_item.setData(self.t_spec, y)

        # Update horizontal threshold lines (map thr to frequency)
        y_thr = fmin + thr * (fmax - fmin)
        self.threshold_line_item.setValue(y_thr)
